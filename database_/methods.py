...
from database_.model import Student, Teacher, Quiz, Class, Record,User
from database_.database import engine
import json
"""增
    用户注册
    学生、教师注册"""
...
def add_quiz_from_json(json_file:str):
    with open(json_file,'r',encoding='utf-8') as f:
        datas = json.load(f)
    for key,data in datas.items():
        quiz = Quiz(id = int(key),content = data['contents'],answer = data['answer'],output=data['output'])
        add(quiz)
        print(f"quiz{key} added")
...
def update_grade_delt(stuid:int,delgrade:int):
    try:
        student = get_data_by_id(Student,stuid)
        if student is None:
            raise ValueError(f"Student with id {stuid} does not exist")
        else:
            with Session(engine) as session:
                student.grade += delgrade
                session.add(student)
                session.commit()
                print("Grade updated successfully!")
    except Exception as e:
        raise ValueError(f"Error when updating grade:{str(e)}")
...
def add_record(*,r_id=None,stu_id:int,quiz_id:int,result = "fail"):
    with Session(engine) as session:
        try:
...
if __name__ == '__main__' :
    #add_record(1,101,101,"success")
    add_quiz_from_json(r'D:\pythonprojects\learn_CNN\questions_json\questions.json')